...
// Sorting displaying transactions deposit withdrawls
...
const displayTransactions = function (account, sort = false) {
  ...
  transacs.forEach(function (trans, index) {
    ...
    <div class="transactions=__type transactions__type--${transType}">
      ${index + 1}${transType} 
    </div>
...
// Making short nicknames for transfers
...
// show total get, pay out, interest
...
const startLogoutTimer = function () {
  const logOutTimerCallback = function () {
    // Every call displaying time UI
    const minutes = String(Math.trunc(time / 60)).padStart(2, '0');
    const seconds = String(time % 60).padStart(2, '0');
    labelTimer.textContent = `${minutes}: ${seconds}`;

    //stop timer, exit from app
    if (time === 0) {
      clearInterval(logOutTimer);
      containerApp.style.opacity = 0;
      labelWelcome.textContent = "Don't worry be happy";
    }
...
//   // transactionsUi;
//   // console.log(transactionsUi.map(elem => +(elem.textContent)));
//   const transactionsUiArray = Array.from(
//     transactionsUi,
...
// Making every 2 transaction grey, why ? I don't know
